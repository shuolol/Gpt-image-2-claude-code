...
function str(
  v: string | boolean | (string | boolean)[] | undefined,
): string | undefined {
...
async function cmdSimple(rawArgs: string[]): Promise<void> {
  ...
  try {
    ...
    exitWithError(
      `Invalid argument: ${err instanceof Error ? err.message : String(err)}`,
    );
  }
  ...
  const imageUrl = (args.values.url as string | undefined)?.trim();
  const prompt = (args.values.prompt as string | undefined)?.trim();

  if (!imageUrl) exitWithError("Missing required option: --url (or -u)");
  if (!prompt) exitWithError("Missing required option: --prompt (or -p)");
...
async function cmdGenerateSingle(
  ...
  try {
    ...
    if (urls.length === 0) exitWithError("API returned no image URLs");
    process.stderr.write("✅ Done!\n\n");
...
async function cmdGenerateImages(
  ...
  if (!productInfoRaw) exitWithError("Missing --product-info");
  if (!configsRaw) exitWithError("Missing --configs");
